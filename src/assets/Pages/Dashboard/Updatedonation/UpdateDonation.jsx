...
const UpdateDonation = () => {
   ...
   // console.log(loadeddata);
    const {_id,address,distric,donationdate,donationtime,group,hospital,recipient,upazila}=loadeddata;
    //console.log(_id)
    const {user}=useAuth()
   ...
   const {
    ...
    //    }
       ...
       axiosPublic.patch(`/blood-request/${_id}`,data)
       .then(res=>{
  ...
  <div className="w-full  p-8 space-y-3 rounded-xl dark:bg-gray-900 dark:text-gray-100">
	<h1 className="text-2xl font-bold text-center">Update  Request</h1>
        ...
        <div className="flex gap-4 ">
			...
			<input defaultValue={recipient} {...register("recipient", { required: true })} type="text" name="recipient" id="recipient" placeholder=" recipient Name" className="w-full px-4 py-3 rounded-md dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:dark:border-violet-400" />
            {errors.recipient && <span>This field is required</span>}
		</div>
		<div className="space-y-1 flex-1 text-sm">
			<label  className="block dark:text-gray-400"> hospital name</label>
			<input defaultValue={hospital} {...register("hospital", { required: true })} type="text" name="hospital" id="hospital" placeholder="hospital name" className="w-full px-4 py-3 rounded-md dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:dark:border-violet-400" />
		...
		<div className="space-y-1 flex-1 text-sm">
			...
			<input defaultValue={address} {...register("address", { required: true })} type="text" name="address" id="username" placeholder="full address line" className="w-full px-4 py-3 rounded-md dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:dark:border-violet-400" />
        ...
        <div className="space-y-1 flex-1 text-sm">
			...
			<input defaultValue={donationdate} {...register("donationdate", { required: true })} type="date" name="donationdate" id="donationdate" placeholder="Donation Date" className="w-full px-4 py-3 rounded-md dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:dark:border-violet-400" />
            {errors.donationdate && <span>This field is required</span>}
		</div>
		<div className="space-y-1 flex-1 text-sm">
			<label  className="block dark:text-gray-400">Donation Time</label>
			<input defaultValue={donationtime} {...register("donationtime", { required: true })} type="time" name="donationtime" id="donationtime" placeholder="Donation Time" className="w-full px-4 py-3 rounded-md dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:dark:border-violet-400" />
     ...
     <h2>Select a District:</h2>
      ...
      {selectedDistrict && (
        ...
        <button  className="block text-center  p-3   from-purple-500 via-purple-600 to-purple-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-purple-300 dark:focus:ring-purple-800 font-medium rounded-lg text-sm px-5 py-2.5  me-2 mb-2"> Update Request</button>
